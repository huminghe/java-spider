...
public abstract class IDAO<T> {
	...
	private boolean initClient(ESClient esClient) {
		...
		logger.debug("检查ES index,type 是否存在");
		return check();
	}
	...
	protected boolean deleteByQuery(QueryBuilder queryBuilder, Task task) {
		...
		while (true) {
			...
			if (response.getHits().getHits().length == 0) {
				...
				logger.debug("按query{}删除数据ID添加完毕,准备执行删除", queryBuilder.toString());
				break;
			} else {
				if (task != null) {
				...
				logger.debug("按query{}删除数据已经添加{}条", queryBuilder.toString(), bulkRequestBuilder.numberOfActions());
			}
		}
		if (bulkRequestBuilder.numberOfActions() <= 0) {
			...
			logger.debug("按query{}删除数据时未找到数据,请检查参数", queryBuilder.toString());
			return false;
		}
	 ...
	 * @return 经过分词的输出流
	 */
	protected void exportData(QueryBuilder queryBuilder, Function<SearchResponse, List<List<String>>> labelsSupplier,
			Function<SearchResponse, List<String>> contentSupplier, OutputStream outputStream) {
		final int size = 50;
		String scrollId = null;
		for (int page = 1;; page++) {
			logger.debug("正在输出{}第{}页", queryBuilder, page);
			SearchResponse response;
			if (StringUtils.isBlank(scrollId)) {
				response = client.prepareSearch(INDEX_NAME).setTypes(TYPE_NAME).setQuery(queryBuilder).setSize(size)
						.setScroll(TimeValue.timeValueMinutes(SCROLL_TIMEOUT)).execute().actionGet();
			...
			try {
				...
				logger.error("输出错误,{}", e.getLocalizedMessage());
			}
		}
	}
}
